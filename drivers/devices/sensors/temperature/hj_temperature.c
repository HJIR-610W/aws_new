...
driver_t *hjTemperature_open(int32_t num, void *opt)
{
  ...
  switch (hjtemp->physical_layer)
  {
    case ePHYSICAL_RS232:
      ...
      drv_uart_init(hj_temperature_cfg.modbus.port_num,&uart_config,"Temperature");
      break;
    case ePHYSICAL_RS485:
      ...
      drv_rs485_init(hj_temperature_cfg.modbus.port_num, &uart_config,"Temperature");
      break;
  }
...
float hjTemperature_read(driver_t *driver, uint8_t *err)
{
  ...
  mb_ret = modbus_read_hold_reg(&cfg->modbus,  HJ_REG_NUM_TEMP, (uint16_t *)reg, 1);

  if(mb_ret != eMODBUS_OK)
  {
    ERROR_PRINTF("hjTemperature_read error %s",get_modbus_err_string(mb_ret));
    *err = DRV_ERR_TIMEOUT;
    temp = NAN;
  }
  else
