...
/**
 * @brief 스크린 행을 포맷팅한후 잔여는 공백으로 하여 완성
 * @param buff 
 */
...
enable_left_long_key();
  ...
  // 최대 자릿수 계산 (음수 고려)
  temp_max = (abs(max) > abs(min)) ? abs(max) : abs(min);
  if (temp_max == 0)
    temp_max = 1;
