...
void draw_rain(win_t *p_win)
{
    ...
    case 0:
  {
    ...
    win_printf_row(p_win, row_count++, "%s: %6.1f", m_l("전일",RAIN_WD),(float)(g_rainfall.yesterday/10.f));

    win_printf_row(p_win, row_count++, "%s: %6.1f", m_l("금일", RAIN_WD), (float)(g_rainfall.today / 10.f));

    win_printf_row(p_win, row_count++, "%s: %6.1f", m_l("1분", RAIN_WD), (float)(g_rainfall.min / 10.f));

    win_printf_row(p_win, row_count++, "%s: %6.1f", m_l("10분", RAIN_WD), (float)(g_rainfall.ten_min / 10.f));

    win_printf_row(p_win, row_count++, "%s: %6.1f", m_l("시간", RAIN_WD), (float)(g_rainfall.hourly / 10.f));

    win_printf_row(p_win, row_count++, "%s: %6.1f", m_l("연간", RAIN_WD), (float)(g_rainfall.yearly / 10.f));

    win_printf_row(p_win, row_count++, "%s: %6.1f", m_l("월간", RAIN_WD), (float)(g_rainfall.monthly / 10.f));
...
void draw_aws(win_t *p_win)
{
  ...
  if (p_kma->temperature.enable)
  {
    ...
    else
    {
      ...
      switch (page)
      {
        ...
        case eAWS_DATA_HOUR:
        case eAWS_DATA_DAY:
          win_printf_row(p_win, row_count++, "%s: %7.1f C   최소:%7.1f C 최대:%7.1f C", m_l("기온", AWS_WD), data,data_min,data_max);
          break;
        case eAWS_DATA_RAW:
        {
           f_data = p_kma->temperature.raw.f;
  ...
  if (p_kma->wind_direction_avg.enable)
  {
    ...
    else
    {
      ...
      switch (page)
      {
        ...
        case eAWS_DATA_HOUR:
        case eAWS_DATA_DAY:
          win_printf_row(p_win, row_count++, "%s: %7.1f 도  최대:%7.1f", m_l("풍향", AWS_WD), data,data_max);
        break;
      case eAWS_DATA_RAW:
           f_data = p_kma->wind_direction_avg.raw.f;
    ...
    else
    {
      switch (page)
      {
        ...
        case eAWS_DATA_DAY:
          ...
          win_printf_row(p_win, row_count++, "%s: %7.1f m/s 최대:%7.1f ", m_l("풍속", AWS_WD), data,data_max);
          break;
        case eAWS_DATA_RAW:
          f_data = p_kma->wind_speed_avg.raw.f;
      ...
      switch (page)
        {
          ...
          case eAWS_DATA_REAL:
          {
             data = p_kma->precipitation_presence.data;
  ...
  if (p_kma->relative_humidity.enable&& (page!=eAWS_DATA_DAY))
  {
    ...
    else
    {
       ...
       {
        case eAWS_DATA_RAW:
          f_data = p_kma->relative_humidity.raw.f;
  ...
  if (p_kma->precipitation_fine.enable)
  {
    ...
    else
    {
      ...
      {
        case eAWS_DATA_RAW:
         f_data = p_kma->precipitation_fine.raw.f;
  ...
  if (p_kma->solar_radiation.enable)
  {
    ...
    else
    {
      switch (page)
      {
        case eAWS_DATA_REAL:
        {
          float solar_radiation = (float)g_solar_radiation.min_acc;
  ...
  if (p_kma->sunshine_duration.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR && page != eAWS_DATA_DAY))
  {
    ...
    else
    {
      switch (page)
      {
        case eAWS_DATA_RAW:
        {
           f_data = p_kma->sunshine_duration.raw.f;
  ...
  if (p_kma->surface_temperature.enable )
  {
    ...
    {
      data = KMA_TO_TEMPERATURE(p_kma->surface_temperature.data);
      data_min = KMA_TO_TEMPERATURE(p_kma->surface_temperature.min);
      data_max = KMA_TO_TEMPERATURE(p_kma->surface_temperature.max);
      ...
      case eAWS_DATA_HOUR:
      case eAWS_DATA_DAY:
        win_printf_row(p_win, row_count++, "%s: %7.1f C   최소:%7.1f C 최대:%7.1f C", m_l("지면온도", AWS_WD), data, data_min, data_max);
        break;
      case eAWS_DATA_RAW:
        f_data = p_kma->surface_temperature.raw.f;
        win_printf_row(p_win, row_count++, "%s: %7.1f C", m_l("지면온도", AWS_WD), f_data);
        break;
      }
  ...
  if (p_kma->grass_temperature.enable )
  {
    ...
    {
      data = KMA_TO_TEMPERATURE(p_kma->grass_temperature.data);
      data_min = KMA_TO_TEMPERATURE(p_kma->grass_temperature.min);
      data_max = KMA_TO_TEMPERATURE(p_kma->grass_temperature.max);
      ...
      case eAWS_DATA_REAL:
        ...
        case eAWS_DATA_HOUR:
        case eAWS_DATA_DAY:
          win_printf_row(p_win, row_count++, "%s: %7.1f C   최소:%7.1f C 최대:%7.1f C", m_l("초상온도", AWS_WD), data, data_min, data_max);
          break;
        case eAWS_DATA_RAW:
          f_data = p_kma->grass_temperature.raw.f;
          win_printf_row(p_win, row_count++, "%s: %7.1f C", m_l("초상온도", AWS_WD), f_data);
          break;
      }
    ...
    else
    {
      ...
      case eAWS_DATA_REAL:
        ...
        case eAWS_DATA_DAY:
          ...
          break;
        case eAWS_DATA_RAW:
          f_data = p_kma->soil_temperature_5cm.raw.f;
          win_printf_row(p_win, row_count++, "%s: %7.1f C", m_l("지중온도 5cm", AWS_WD), f_data);
          break;
      }
    }
  }
  ...
  if (p_kma->soil_temperature_10cm.enable)
  {
    ...
    else
    {
      ...
      case eAWS_DATA_DAY:
        ...
        break;
      case eAWS_DATA_RAW:
        f_data = p_kma->soil_temperature_10cm.raw.f;
        win_printf_row(p_win, row_count++, "%s: %7.1f C", m_l("지중온도 10cm", AWS_WD), f_data);
        break;
      }
    }
  }
  ...
  if (p_kma->soil_temperature_20cm.enable )
  {
    ...
    else
    {
      ...
      case eAWS_DATA_DAY:
        ...
        break;
      case eAWS_DATA_RAW:
        f_data = p_kma->soil_temperature_20cm.raw.f;
        win_printf_row(p_win, row_count++, "%s: %7.1f C", m_l("지중온도 20cm", AWS_WD), f_data);
        break;
      }
    }
  }
  ...
  if (p_kma->soil_temperature_30cm.enable )
  {
    ...
    else
    {
      ...
      case eAWS_DATA_DAY:
        ...
        break;
      case eAWS_DATA_RAW:
        f_data = p_kma->soil_temperature_30cm.raw.f;
        win_printf_row(p_win, row_count++, "%s: %7.1f C", m_l("지중온도 20cm", AWS_WD), f_data);
        break;
      }
    }
  ...
  if (p_kma->soil_temperature_50cm.enable )
  {
    ...
    else
    {
      ...
      case eAWS_DATA_DAY:
        ...
        break;
      case eAWS_DATA_RAW:
        f_data = p_kma->soil_temperature_50cm.raw.f;
        win_printf_row(p_win, row_count++, "%s: %7.1f C", m_l("지중온도 50cm", AWS_WD), f_data);
        break;
      }
    }
  }
  ...
  if (p_kma->soil_temperature_1m.enable )
  {
    ...
    else
    {
      ...
      case eAWS_DATA_DAY:
        ...
        break;
      case eAWS_DATA_RAW:
        f_data = p_kma->soil_temperature_1m.raw.f;
        win_printf_row(p_win, row_count++, "%s: %7.1f C", m_l("지중온도 1m", AWS_WD), f_data);
        break;
      }
    }
  }
  ...
  if (p_kma->soil_temperature_1_5m.enable )
  {
    ...
    else
    {
      ...
      case eAWS_DATA_DAY:
        ...
        break;
      case eAWS_DATA_RAW:
        f_data = p_kma->soil_temperature_1_5m.raw.f;
        win_printf_row(p_win, row_count++, "%s: %7.1f C", m_l("지중온도 1.5m", AWS_WD), f_data);
        break;
      }
    }
  }
  ...
  if (p_kma->soil_temperature_3m.enable )
  {
    ...
    else
    {
      ...
      case eAWS_DATA_DAY:
        ...
        break;
      case eAWS_DATA_RAW:
        f_data = p_kma->soil_temperature_3m.raw.f;
        win_printf_row(p_win, row_count++, "%s: %7.1f C", m_l("지중온도 3m", AWS_WD), f_data);
        break;
      }
    }
  }
  ...
  if (p_kma->soil_temperature_5m.enable )
  {
    ...
    else
    {
      ...
      case eAWS_DATA_DAY:
        ...
        break;
      case eAWS_DATA_RAW:
        f_data = p_kma->soil_temperature_5m.raw.f;
        win_printf_row(p_win, row_count++, "%s: %7.1f C", m_l("지중온도 5m", AWS_WD), f_data);
        break;
      }
    }
  }
  ...
  if (p_kma->cloud_height_1st.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      ...
      {
         f_data = p_kma->cloud_height_1st.raw.f;
  ...
  if (p_kma->cloud_height_2nd.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->cloud_height_2nd.raw.f;
  ...
  if (p_kma->cloud_height_3rd.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->cloud_height_3rd.raw.f;
  ...
  if (p_kma->cloud_amount.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->cloud_amount.raw.f;
  ...
  if (p_kma->pm10_concentration.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->pm10_concentration.raw.f;
  ...
  if (p_kma->pm25_concentration.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->pm25_concentration.raw.f;
  ...
  if (p_kma->net_radiation.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->net_radiation.raw.f;
        win_printf_row(p_win, row_count++, "%s: %7.2f W/m2", m_l("순복사", AWS_WD), f_data);
      }
      else if (page == eAWS_DATA_REAL || page == eAWS_DATA_1MIN)
      {
  ...
  if (p_kma->reflected_radiation.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->reflected_radiation.raw.f;
  ...
  if (p_kma->direct_radiation.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->direct_radiation.raw.f;
  ...
  if (p_kma->current_weather.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->current_weather.raw.f;
  ...
  if (p_kma->soil_moisture_10cm.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->soil_moisture_10cm.raw.f;
  ...
  if (p_kma->soil_moisture_20cm.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->soil_moisture_20cm.raw.f;
  ...
  if (p_kma->soil_moisture_30cm.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->soil_moisture_30cm.raw.f;
  ...
  if (p_kma->soil_moisture_50cm.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->soil_moisture_50cm.raw.f;
  ...
  if (p_kma->illuminance.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->illuminance.raw.f;
  ...
  if (p_kma->wind_speed_1_5m.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->wind_speed_1_5m.raw.f;
  ...
  if (p_kma->wind_speed_4m.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->wind_speed_4m.raw.f;
  ...
  if (p_kma->instant_wind_speed_1_5m.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->instant_wind_speed_1_5m.raw.f;
  ...
  if (p_kma->instant_wind_speed_4m.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->instant_wind_speed_4m.raw.f;
  ...
  if (p_kma->temperature_0_5m.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->temperature_0_5m.raw.f;
  ...
  if (p_kma->temperature_4m.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->temperature_4m.raw.f;
  ...
  if (p_kma->humidity_0_5m.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->humidity_0_5m.raw.f;
  ...
  if (p_kma->humidity_4m.enable && (page != eAWS_DATA_10MIN && page != eAWS_DATA_HOUR))
  {
    ...
    else
    {
      if (page == eAWS_DATA_RAW)
      {
         f_data = p_kma->humidity_4m.raw.f;
