...
void draw_system(win_t* p_win)
{
  ...
  switch (page)
  {
    case 0:
    {
      ...
      snprintf(buff, sizeof(buff), "%-*s: %s", SYSTEM_WD, "문 상태",
      ...
      snprintf(buff, sizeof(buff), "%-*s: %s", SYSTEM_WD, "저장 기능", message);
      win_print_row(p_win, row_count++, buff);

      snprintf(buff, sizeof(buff), "%-*s: %.1f", SYSTEM_WD, "장비 전원V", bsp_read_battery());
      win_print_row(p_win, row_count++, buff);

      snprintf(buff, sizeof(buff), "%-*s: %.1f", SYSTEM_WD, "장비 온도C", bsp_read_temperature());
      win_print_row(p_win, row_count++, buff);

      if (get_config_app()->ac_use)
      {
        snprintf(buff, sizeof(buff), "%-*s: %s", SYSTEM_WD, "AC","정상");
        win_print_row(p_win, row_count++,  buff);
      }
...
void draw_direct(win_t *p_win)
{
  ...
  switch (page)
  {
    case 0:
    {
      ...
      // 링크 상태
      snprintf(buff, sizeof(buff), "%-*s: %s", DIRECT_WD, "링크",
               ITEM_LIST(get_direct_system()->link_status, linkStatusList));
      win_print_row(p_win, row_count++, buff);

      // 타임아웃 (남은 시간)
      remain_sec = (uint32_t)(get_direct_system()->linkdown_remain_ms / 1000.0);
      snprintf(buff, sizeof(buff), "%-*s: %ds", DIRECT_WD, "타임아웃", remain_sec);
      win_print_row(p_win, row_count++, buff);

      // 송신 카운트
      snprintf(buff, sizeof(buff), "%-*s: %d", DIRECT_WD, "송신", get_direct_system()->tx_cnt);
      win_print_row(p_win, row_count++, buff);

      // 수신 카운트
      snprintf(buff, sizeof(buff), "%-*s: %d", DIRECT_WD, "수신", get_direct_system()->rx_cnt);
      win_print_row(p_win, row_count++, buff);
      ...
      if (last_time == 0)
      {
        snprintf(buff, sizeof(buff), "%-*s: -", DIRECT_WD, "R시간");
      }
      else
      {
