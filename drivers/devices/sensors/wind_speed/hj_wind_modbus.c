...
int32_t hj_wind_direction_init(void *opt)
{
  ...
  g_hj_wind_dir_cfg.modbus.modebus_type = eMODBUS_RS485;
  g_hj_wind_dir_cfg.modbus.port_num = rs485_num_to_driver_num(ott->port);
  g_hj_wind_dir_cfg.modbus.id = ott->modbus_id;

  modbus_init();
  drv_rs485_init(g_hj_wind_dir_cfg.modbus.port_num,&uart_config,"Wind Speed");

  g_hj_wind_dir_driver.cfg = &g_hj_wind_dir_cfg;
  g_hj_wind_dir_driver.api = &g_hj_wind_dir_api;
  g_hj_wind_dir_driver.opened = true;
...
float hj_wind_direction_read(uint8_t *err)
{
  ...
  ret = modbus_read_input_reg(&g_hj_wind_dir_inst.modbus,  REG_R_WIND_DATA, reg, _countof(reg));
