...
void stm32_spi_init(SPI_HandleTypeDef *hspi)
{
  if(hspi->Instance == SPI1)
  {
    ...
    hspi1.Init.BaudRatePrescaler = get_spi_prescaler(hspi,1312500);;//고정된 분주비라서 원하는데오 딱 안떨어짐1312500
...
void driver_spi_send_byte(driver_t *drv, uint8_t value)
{
...
void driver_spi_send_bytes(driver_t *drv,uint8_t *data,uint16_t dataLen)
{
...
uint8_t driver_spi_read_byte(driver_t *drv)
{
  HAL_StatusTypeDef status = HAL_OK;
...
uint8_t driver_spi_read_bytes(driver_t *drv,uint8_t *p_buff,uint16_t read_len)
{
  HAL_StatusTypeDef status = HAL_OK;
  spi_cfg_t *cfg = (spi_cfg_t *)drv->cfg;
