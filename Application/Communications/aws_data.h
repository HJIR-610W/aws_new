...
typedef struct
{
  ...
  // 2. 풍향 (1분 평균)
  aws_data_t wind_direction_avg;  // 사용비트: 11, 유효범위: 0 ~ 4095 (인치 코드), 표현범위: 0 ~
                                  // 3599 (관측값 * 10)

  // 3. 풍속 (1분 평균)
  aws_data_t wind_speed_avg;  // 사용비트: 9, 유효범위: 0 ~ 1023 (인치 코드), 표현범위: 0 ~ 1000
                              // (관측값 * 10)

  // 4. 풍향 (1분 순간)
  aws_data_t wind_direction_instant;  // 사용비트: 11, 유효범위: 0 ~ 4095 (인치 코드), 표현범위: 0 ~
                                      // 3599 (관측값 * 10)

  // 5. 풍속 (1분 순간)
  aws_data_t wind_speed_instant;  // 사용비트: 9, 유효범위: 0 ~ 1023 (인치 코드), 표현범위: 0 ~ 1000
                                  // (관측값 * 10)

  // 6. 강수량 (0.5/1.0 mm)
  aws_data2_t precipitation;  // 사용비트: 14, 유효범위: 0 ~ 32767 (인치 코드), 표현범위: 0 ~ 32767

  // 7. 기압 (1분 평균 현지 기압)
  aws_data_t pressure;  // 사용비트: 13, 유효범위: 0 ~ 16383 (인치 코드), 표현범위: 5000 ~ 11000

  // 8. 강수 유무
  aws_data_t precipitation_presence;  // 사용비트: 3, 유효범위: 0 ~ 15 (인치 코드), 표현범위: 0 =
                                      // 강수 없음, 1 = 강수 있음
  ...
  // 10. 상대습도 (1분 평균)
  aws_data_t relative_humidity;  // 사용비트: 9, 유효범위: 0 ~ 1023 (인치 코드), 표현범위: 0 ~ 1000
                                 // (관측값 * 10)
  ...
  // 1. 일사 (누적값)
  aws_data_t solar_radiation;  // 사용비트: 14, 유효범위: 0 ~ 32767 (인치 코드), 표현범위: 0 ~ 32767
                               // [관측값(MJ/m²) * 100]

  // 2. 일조 (누적 시간)
  aws_data_t sunshine_duration;  // 사용비트: 15, 유효범위: 0 ~ 65535 (인치 코드), 표현범위: 0 ~
                                 // 65535 [누적시간(초 단위)]

  // 3. 지면온도 (1분 평균)
  aws_data_t surface_temperature;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500 ~
                                   // 2000 [(관측값 + 100) * 10]

  // 4. 초상온도 (1분 평균)
  aws_data_t grass_temperature;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500 ~
                                 // 2000 [(관측값 + 100) * 10]

  // 5. 지중온도 (5cm, 1분 평균)
  aws_data_t soil_temperature_5cm;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500 ~
                                    // 2000 [(관측값 + 100) * 10]

  // 6. 지중온도 (10cm, 1분 평균)
  aws_data_t soil_temperature_10cm;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500
                                     // ~ 2000 [(관측값 + 100) * 10]

  // 7. 지중온도 (20cm, 1분 평균)
  aws_data_t soil_temperature_20cm;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500
                                     // ~ 2000 [(관측값 + 100) * 10]

  // 8. 지중온도 (30cm, 1분 평균)
  aws_data_t soil_temperature_30cm;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500
                                     // ~ 2000 [(관측값 + 100) * 10]

  // 9. 지중온도 (50cm, 1분 평균)
  aws_data_t soil_temperature_50cm;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500
                                     // ~ 2000 [(관측값 + 100) * 10]

  // 10. 지중온도 (1.0m, 1분 평균)
  aws_data_t soil_temperature_1m;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500 ~
                                   // 2000 [(관측값 + 100) * 10]

  // 11. 지중온도 (1.5m, 1분 평균)
  aws_data_t soil_temperature_1_5m;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500
                                     // ~ 2000 [(관측값 + 100) * 10]

  // 12. 지중온도 (3.0m, 1분 평균)
  aws_data_t soil_temperature_3m;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500 ~
                                   // 2000 [(관측값 + 100) * 10]

  // 13. 지중온도 (5.0m, 1분 평균)
  aws_data_t soil_temperature_5m;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500 ~
                                   // 2000 [(관측값 + 100) * 10]

  // 1. 1층 운고 (1분 평균)
  aws_data_t cloud_height_1st;  // 사용비트: 12, 유효범위: 0 ~ 8191 (인치 코드), 표현범위: 0 ~ 8000
                                // (관측값[m])

  // 2. 2층 운고 (1분 평균)
  aws_data_t cloud_height_2nd;  // 사용비트: 12, 유효범위: 0 ~ 8191 (인치 코드), 표현범위: 0 ~ 8000
                                // (관측값[m])

  // 3. 3층 운고 (1분 평균)
  aws_data_t cloud_height_3rd;  // 사용비트: 12, 유효범위: 0 ~ 8191 (인치 코드), 표현범위: 0 ~ 8000
                                // (관측값[m])

  // 4. 운량
  aws_data_t cloud_amount;  // 사용비트: 3, 유효범위: 0 ~ 15 (인치 코드), 표현범위: 0 ~ 10 (관측값)
  ...
  // 6. PM10 (분진농도)
  aws_data_t pm10_concentration;  // 사용비트: 11, 유효범위: 0 ~ 4095 (인치 코드), 표현범위: 1 ~
                                  // 3599 (관측값 [μg/m³] × 10)

  // 7. PM2.5 (분진농도)
  aws_data_t pm25_concentration;  // 사용비트: 11, 유효범위: 0 ~ 4095 (인치 코드), 표현범위: 1 ~
                                  // 3599 (관측값 [μg/m³] × 10)

  // 8. 순복사 (1분 평균)
  aws_data_t net_radiation;  // 사용비트: 14, 유효범위: 0 ~ 32767 (인치 코드), 표현범위: 0 ~ 32767
                             // (관측값[W/m²] + 1000) × 10

  // 9. 전천복사 (1분 평균)
  aws_data_t total_radiation;  // 사용비트: 14, 유효범위: 0 ~ 32767 (인치 코드), 표현범위: 0 ~ 32767
                               // (관측값[W/m²] + 1000) × 10

  // 10. 반사복사 (1분 평균)
  aws_data_t reflected_radiation;  // 사용비트: 14, 유효범위: 0 ~ 32767 (인치 코드), 표현범위: 0 ~
                                   // 32767 (관측값[W/m²] + 1000) × 10

  // 11. 직달복사 (1분 평균)
  aws_data_t direct_radiation;  // 사용비트: 14, 유효범위: 0 ~ 32767 (인치 코드), 표현범위: 0 ~
                                // 32767 (관측값[W/m²] + 1000) × 10
  ...
  // 1. 토양수분 (10 cm)
  aws_data_t soil_moisture_10cm;  // 사용비트: 9, 유효범위: 0 ~ 1023 (인치 코드), 표현범위: 0 ~ 1000
                                  // (관측값 * 10)

  // 2. 토양수분 (20 cm)
  aws_data_t soil_moisture_20cm;  // 사용비트: 9, 유효범위: 0 ~ 1023 (인치 코드), 표현범위: 0 ~ 1000
                                  // (관측값 * 10)

  // 3. 토양수분 (30 cm)
  aws_data_t soil_moisture_30cm;  // 사용비트: 9, 유효범위: 0 ~ 1023 (인치 코드), 표현범위: 0 ~ 1000
                                  // (관측값 * 10)

  // 4. 토양수분 (50 cm)
  aws_data_t soil_moisture_50cm;  // 사용비트: 9, 유효범위: 0 ~ 1023 (인치 코드), 표현범위: 0 ~ 1000
                                  // (관측값 * 10)

  // 5. 조도량 (1분 평균)
  aws_data_t illuminance;  // 사용비트: 14, 유효범위: 0 ~ 32767 (인치 코드), 표현범위: 0 ~ 32767
                           // (관측값 * 100)

  // 6. 풍속 (1.5 m, 1분 평균)
  aws_data_t wind_speed_1_5m;  // 사용비트: 9, 유효범위: 0 ~ 1023 (인치 코드), 표현범위: 0 ~ 1000
                               // (관측값 * 10)

  // 7. 풍속 (4.0 m, 1분 평균)
  aws_data_t wind_speed_4m;  // 사용비트: 9, 유효범위: 0 ~ 1023 (인치 코드), 표현범위: 0 ~ 1000
                             // (관측값 * 10)

  // 8. 순간 풍속 (1.5 m)
  aws_data_t instant_wind_speed_1_5m;  // 사용비트: 9, 유효범위: 0 ~ 1023 (인치 코드), 표현범위: 0 ~
                                       // 1000 (관측값 * 10)

  // 9. 순간 풍속 (4.0 m)
  aws_data_t instant_wind_speed_4m;  // 사용비트: 9, 유효범위: 0 ~ 1023 (인치 코드), 표현범위: 0 ~
                                     // 1000 (관측값 * 10)

  // 10. 기온 (0.5 m)
  aws_data_t temperature_0_5m;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500 ~
                                // 1500 [(관측값 + 100) * 10]

  // 11. 기온 (4.0 m)
  aws_data_t temperature_4m;  // 사용비트: 10, 유효범위: 0 ~ 2047 (인치 코드), 표현범위: 500 ~ 1500
                              // [(관측값 + 100) * 10]

  // 12. 습도 (0.5 m, 1분 평균)
  aws_data_t humidity_0_5m;  // 사용비트: 9, 유효범위: 0 ~ 1023 (인치 코드), 표현범위: 0 ~ 1000
                             // (관측값 * 10)
