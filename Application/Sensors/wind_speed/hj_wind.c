...
float read_hjWind(void *driver, uint8_t channel, uint8_t *err)
{
  ...
  // 10배된 값이 수신됨됨
  len = driver_rs485_recv(cfg->rs485_io, recv, sizeof(recv),
                          50);  // 독라이트 테스트시 50ms이상은 되어야함함
...
driver_t *hjwind_open(uint8_t num, void *opt)
{
  ...
  hjWind_cfg.rs485_io = driver_rs485_open((int)hjwind_config->rs485_port, &uart_config);
...
void hjwind_get(driver_t *drv, int cmd, void *val) {}
