 ...
 HAL_StatusTypeDef ReceivePacket(uint8_t *p_data, uint32_t *p_length, uint32_t timeout)
{
    ...
    if(debug_recv(&char1,1,timeout))
    {
        switch (char1)
        {
            ...
            case EOT:
                ...
                break;
            case CA:
                if(debug_recv(&char1,1,timeout)&&(char1==CA))
                {
                    packet_size = 2;
    ...
    *p_data = char1;
    ...
    {
        status = HAL_ERROR;
        len = debug_recv(&p_data[PACKET_NUMBER_INDEX],packet_size + PACKET_OVERHEAD_SIZE,timeout);
...
COM_StatusTypeDef Ymodem_Transmit (uint8_t *p_buf, const uint8_t *p_file_name, uint32_t file_size)
{
  ...
  //  HAL_UART_Transmit(&UartHandle, &aPacketData[PACKET_START_INDEX], PACKET_SIZE + PACKET_HEADER_SIZE, NAK_TIMEOUT);
    ...
    if(debug_recv(&a_rx_ctrl[0], 1, NAK_TIMEOUT))
    {
      if (a_rx_ctrl[0] == ACK)
      {
        ack_recpt = 1;
    ...
    if(debug_recv(&a_rx_ctrl[0], 1, NAK_TIMEOUT) && (a_rx_ctrl[0] == CA))
        {
          HAL_Delay( 2 );
  ...
  while ((size) && (result == COM_OK ))
  {
    ...
    //  HAL_UART_Transmit(&UartHandle, &aPacketData[PACKET_START_INDEX], pkt_size + PACKET_HEADER_SIZE, NAK_TIMEOUT);
     ...
     // if ((HAL_UART_Receive(&UartHandle, &a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK) && (a_rx_ctrl[0] == ACK))
      if(debug_recv(&a_rx_ctrl[0], 1, NAK_TIMEOUT)&& (a_rx_ctrl[0] == ACK))
      {
        ack_recpt = 1;
        if (size > pkt_size)
        {
        ...
        while ((file_done == 0) && (result == COM_OK))
        {
            switch (ReceivePacket(aPacketData, &packet_length, DOWNLOAD_TIMEOUT))
            {
                ...
                default:
                    ...
                    {
                        osDelay(10);
                        debug_put_ch(CRC16); /* Ask for a packet */
                    }
                    break;
                }
            }
