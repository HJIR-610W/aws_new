...
float read_pt100_temperature(driver_t *driver,uint8_t *err)
{
  ...
  //?¨Í∏∞??Î∂???Í∏∞Ï°¥ AWS ?®ÎèÑ ÏΩîÎìú?∏Îç∞ ?¥Ìï¥Í∞? ?àÎê®.?ºÎã® ?¨Ïö©
  //?êÎ∂Ñ?? sSpan?? -40,60??ADCÍ∞?
  sSpan    = fullset - offset;   //????1Í∞úÎãπ ADCÍ∞?,79%,77%Î°?offset %Ï∂úÏ≤ò Î™®Î¶ÑÎ¶?
  sMinus45 = (int32_t)(((float)sSpan / 38.97) * 0.79);//??Î∂ÑÏÑù:38.97= 123.24-84.27
  sPlus65  = (int32_t)(((float)sSpan / 38.97) * 0.77);

  sSpan   = ((fullset + sMinus45) - (offset - sPlus65));//Î≤îÏúÑÎ•????ìÍ≤å Ï§?
     ...
     (sAdval < ((fullset+ sPlus65) + errTmp)))
  {
    if(sSpan > 0)
    {
      x = 40.53 / (float)sSpan;     //40.53= 124.01(62??-83.48(-42??,  AD Convertion Value Í∞íÏùÑ ?????åÏù¥Î∏îÏóê ÎßûÏ∂§
      resistance = (float)(sAdval - (offset- sMinus45)) * x + pt100_table[0]; //  ????∞í Í≤????òÍ∏∞?ÑÌï¥ ?ÑÏïï??????úºÎ°?Î≥???
      //f??????∞í
      temperature = pt100_resistance_to_temperature(resistance);
    }
    else
