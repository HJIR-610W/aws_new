    ...
    if (g_journal_record.is_valid == 0xAA) {
        ...
        if (SD_write_sector(lba, data)) {
            ...
            if (journal_commit()) {
                ...
                //printf("Journal Recovery Commit Failed.\n");
                return false; 
            }
        } else {
            // 복구 쓰기 실패 (SD 카드 드라이버 문제).
            // 데이터는 FRAM에 그대로 남아있어 다음 리셋 시 다시 시도될 것입니다.
            //printf("Journal Recovery Write to SD Card Failed.\n");
            return false;
        }
    } else {
        // 유효한 백업본이 없으므로 복구할 필요가 없습니다 (is_valid == 0xFF).
        //printf("No valid journal backup found. System OK.\n");
        return true;
    }
}
